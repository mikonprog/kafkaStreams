...
internal class ListingByPostcodeProcessorTest {
    ...
    fun setup() {
        ...
        val processor = ListingByPostcodeProcessor(mapper, stream)

        processor.listingByPostcodeProcessor()
    ...
    fun `Listing is added to the store`() {
        ...
        val listingsByPostcodeStore =
                testDriver.getKeyValueStore<String, Set<ListingSummary>>("listing-by-postcode")

        val listings = listingsByPostcodeStore.get(storeKey)

        assertThat(listings).hasSize(1)
        assertThat(listings.first().latitude).isEqualTo(51.48655)
        assertThat(listings.first().longitude).isEqualTo(-0.189434)
    }
}
